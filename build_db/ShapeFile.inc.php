...
*   15 seconds loading a 210907 points shape file
	...
	* Example:
	...
	$shp = new ShapeFile("../../viz/shapefile/file.shp",$options); 
...
class ShapeFile{
	...
	*/
	...
	public function getDBFHeader(){
	  $dbf = dbase_open($this->dbf_filename , 0);
	  // solo en PHP5 $dbf_data = dbase_get_header_info($dbf);
	  $dbf_header_data = dbase_get_header_info($dbf);
	  dbase_close($dbf);
	  return $dbf_header_data;
	  /*	  
		$dbf_header_data = array();
		if(is_readable($dbf_header_data)){
			$dbf = dbase_open($this->dbf_filename , 1);
			// solo en PHP5 $dbf_data = dbase_get_header_info($dbf);
			$dbf_header_data = dbase_get_header_info($dbf);
			dbase_close($dbf);
			return $dbase_header_data;
		}
		else echo "d______baseh___header not readable<br/>";
		return NULL;
		*/
	}
