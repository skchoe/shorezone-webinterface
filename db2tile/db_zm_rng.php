...
for ($count=$initCount; $count <= $endCount ; $count = $count + 1) {
  ...
  $zmlist = computeTileZoomRng($source_gri);
  $zmsize = computeTileImageSizeByZoom($wss_db, $source_gri, 11);
  $zmcount = computeTileCountByZoom($wss_db, $source_gri, 11);
  if($zmlist == FALSE)  
    echo "Species_id: [$source_gri] returns NULL <br />";
  else
    echo "Species_id: [$source_gri] returns ".$zmlist."(".sizeof($zmlist)."), min(".min($zmlist)."), max(".max($zmlist).").  \t folder size z(11)=".$zmsize."KB \t #file(11) = ".$zmcount."<br />";
 }
