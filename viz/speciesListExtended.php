...
$query_kind = "SELECT source_gri, cm_name, sc_name, taxon_grou, animal_for, has_no_hab, taxon_gr_1 FROM ".$species_feature_tbl;
...
<?php
function echoByTaxonGroup($dst_tbl, $result_kind, $taxon_grou) {
  ...
  for ($count=0; $count < mysql_numrows($result_kind); $count++) {
    ...
    if($tgrp_symbol == $taxon_grou) {
      ...
      //echo "<a href=\"http://155.97.130.88/wss_maps/viz/speciesExtent.php?species=".$source_gri."\"> ".$n_cm." / ".$n_sc."</a><br>";
      echo "<a href=\"http://http://firenze.lib.utah.edu/westernsoundscape.org/maps/map_extends/viz/speciesExtent.php?species=".$source_gri."\"> <".$count."> [".$source_gri."] ".$n_cm." / ".$n_sc."</a><br>";
/*
      if(available($dst_tbl, $source_gri) == TRUE)
        echo "<a href=\"http://155.97.130.88/wss_maps/viz/speciesExtent.php?species=".$source_gri."\"> [".$source_gri."] ".$n_cm." / ".$n_sc."</a><br>";
      else
      echo "XXXXXXXXXX[".$source_gri."] ".$n_cm." / ".$n_sc.": Map is not availa
ble now.</a><br>";
*/
    }
  }
...
function available($dst_tbl, $src_id) {
  ...
  if($result_id == FALSE) {
    echo $src_id." is not available<br/>";
    return FALSE;
  }
  else  {
    echo $src_id." is available<br/>";
    return TRUE;
  }
}
